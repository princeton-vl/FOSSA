...
# Compute z axis depth of each pixel from distance of each pixel from camera
...
class Hypersim(BaseDataset):
    def __init__(self, args, mode):
        if isinstance(args, dict): # Convert the args that was serialized to a dict back to argparse.Namespace
        ...
        self.dataset_max_depth_value = args.hypersim_max_depth_to_supervise_on
        ...
        self.min_valid_train_depth = args.min_valid_train_depth
    ...
    def __getitem__(self, idx):
        ...
        # Flips y and z coordinates
        M_cam_T_hcam = np.array([[1., 0., 0.],
                                 [0., -1., 0.],
                                 [0., 0., -1.]])

        # Keep focal lengths (0,0) and (1,1), as well as x and y principal offsets
        ...
        infinite_vals_mask = np.logical_or.reduce([np.logical_not(np.isfinite(distance_meters)), np.isnan(distance_meters), depth < self.min_valid_train_depth])
        depth_valid_mask[infinite_vals_mask] = False

        depth[np.logical_not(depth_valid_mask)] = 1000 # set invalid depth to 1000

        if ((depth > self.dataset_max_depth_value) & (depth < 1000)).any():
            print(f"Warning: depth value greater than dataset_max_depth_value: {self.dataset_max_depth_value}m found. Clipping depth values greater than {self.dataset_max_depth_value}m to be equal to {self.dataset_max_depth_value}m")
        depth_valid_mask[depth > self.dataset_max_depth_value] = False # Do not supervise on depth values greater than dataset_max_depth_value
        ...
        rgb, dep, K, depth_valid_mask, rgb_np_raw, _ = self.process(rgb, dep, K, depth_valid_mask)
